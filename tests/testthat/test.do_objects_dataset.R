...
test_that(".do_objects_dataset: no max coordinates", {
  ...
  expect_equal(res$notif.text, "notif.objects.ok")
  expect_equal(dim(res$data), c(100, 20))
  expect_equal(colnames(res$data), c("id", "square_x", "square_y", "xmin", "ymin",  "zmin", "layer", "object_type", "object_class_size", "year", "object_lithic_type", "xmax", "ymax","zmax", "location_mode", "x", "y", "z", "square", "layer_color"))
  expect_equal(c(sapply(res$data[1, ], typeof), use.names=F),
               c("integer", "integer", "integer", "integer", "integer", "double", "integer", "character", "character","integer", "character","integer", "integer", "double", "character", "integer", "integer", "double", "character", "integer"))
})
