...
Les tableaux doivent être au format .csv et la première ligne doit contenir les noms des variables (le symbole séparateur du csv peut être défini dans l'interface). Les contenus au format html sont autorisés. Cela permet notamment d'introduire références vers des ressources complémentaires du jeu de données (par .ex l'identifiant unique de l'objets dans une autre base de données, ou ceux des concepts d'ontologies employés pour décrire l'objet, etc.).
